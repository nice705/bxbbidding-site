...
def extract_list_items(html: str, base_url: str) -> list:
    ...
    for href, text in link_pattern.findall(html):
        ...
        # 标签优先：带「发布时间」标签的日期优先
        date_str = ""
        label_match = re.search(r'(?:发布时间|发布日期|公告时间|信息时间)[：:]?\s*(\d{4}[-/]\d{1,2}[-/]\d{1,2})', context_plain)
        if label_match:
            date_str = label_match.group(1).replace("/", "-")
        else:
            for m in re.finditer(r'(\d{4}[-/]\d{2}[-/]\d{2})', context_plain):
                pos = m.start()
                if pos > 0 and context_plain[pos-1].isalpha():
                    continue
...
def extract_detail_from_html(html: str, url: str, region_hint: str) -> dict:
    ...
    # 日期（标签优先，排除字母前缀的日期编号）
    date_str = ""
    label_match = re.search(r'(?:发布时间|发布日期|公告时间|信息时间)[：:]?\s*(\d{4}[-/]\d{1,2}[-/]\d{1,2})', text[:1000])
    if label_match:
        date_str = label_match.group(1).replace("/", "-")
    else:
        for m in re.finditer(r'(\d{4}[-/]\d{2}[-/]\d{2})', text[:500]):
            pos = m.start()
